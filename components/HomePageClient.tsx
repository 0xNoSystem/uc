...
const createDefaultSelectedColors = (products: Product[]) =>
...
export default function HomePageClient({
  ...
  const [selectedColors, setSelectedColors] = useState<Record<string, string>>(
    () => createDefaultSelectedColors(products),
  );
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    const storedSelections = window.localStorage.getItem("uc-selected-colors");
    const frame = window.requestAnimationFrame(() => {
      if (storedSelections) {
        try {
  ...
  const incrementProduct = (productId: string, color?: string) => {
    const appliedColor =
      ...
      getDefaultColor(productId, products);
    setCart((prev) => {
            ...
            {products.map((product) => (
              <ProductCard
                ...
                selectedColor={
                  ...
                  getDefaultColor(product.id, products)
                }
