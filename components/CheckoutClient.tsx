...
import type { StoreSettings, StoreProduct } from "@/lib/store";
...
export default function CheckoutClient({
  ...
  useEffect(() => {
    ...
    const nextCart = parseStoredCart(
      window.localStorage.getItem("uc-cart"),
      products,
    );
  ...
  const subtotalBeforeDiscountRaw = cartDetails.reduce(
    (sum, item) => sum + parsePrice(item.product.price) * item.quantity,
    0,
  );

  const subtotalAfterDiscountRaw = cartDetails.reduce(
    (sum, item) =>
      sum +
  ...
  const subtotalBeforeDiscount = roundCurrency(subtotalBeforeDiscountRaw);
  const subtotalAfterDiscount = roundCurrency(subtotalAfterDiscountRaw);

  const discountAmount = Math.max(
    0,
    roundCurrency(subtotalBeforeDiscount - subtotalAfterDiscount),
  );

  const shippingFee =
    subtotalAfterDiscount === 0
      ? 0
      : subtotalAfterDiscount > settings.shippingFreeThreshold
        ? 0
        : settings.shippingFee;
  const orderTotal = roundCurrency(subtotalAfterDiscount + shippingFee);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const paymentMethod = getField("paymentMethod") || "Cash On Delivery";
  ...
  const handlePassOrder = async () => {
    ...
    try {
      ...
      const getErrorMessage = (
        ...
        if (value && "error" in value && typeof value.error === "string") {
          return value.error;
        }
        return "We couldn't send the order email. Please try again.";
      };
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="rounded-2xl border border-white/15 bg-white/5 p-4 text-sm text-white/70">
                  ...
                  <p className="mt-2">
                    ...
                    or call{" "}
                    <span className="text-white">{settings.supportPhone}</span>.
                  </p>
                </div>
              </div>
              <div>
              ...
              <div className="mt-6 space-y-4">
                {cartDetails.length === 0 ? (
                  <p className="text-sm text-white/60">
                    Your cart is empty. Head back and add a product before
                    checking out, you silly goose.
                  </p>
                ) : (
          ...
          <div className="w-full max-w-xl rounded-3xl border border-white/15 bg-black/90 p-6 text-white shadow-2xl">
            ...
            <div className="mt-6 space-y-4 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm">
              ...
              {confirmationData.notes ? (
                <p className="text-white/70">Notes: {confirmationData.notes}</p>
              ) : null}
              <p className="text-white/80">
                Payment: {confirmationData.paymentMethod || "Cash On Delivery"}
              </p>
              <p className="text-xs text-white/60">
                {confirmationData.email
